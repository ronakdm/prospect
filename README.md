...
in which $\ell_i(w)$ denotes the loss of a model with weights $w \in \mathbb{R}^d$ on data instances $i$, $D_{\chi^2}(q\Vert \mathbf{1}_n/n)$ is the ${\chi}^2$-divergence between some distribution $q$ and the uniform distribution.
